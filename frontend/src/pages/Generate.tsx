...
const Generate = () => {
  ...
  const handleGenerate = async () => {
    if(!isLoggedIn) return toast.error('You need to be logged in to generate thumbnails')
...
<ColorSchemeSelector value={colorSchemeId} onChange={setColorSchemeId} />
 ...
 py-3 rounded-lg border border-white/10 bg-white/6 text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                ...
                {/* button */}
                {!id && (
                  <button onClick={handleGenerate} className=" text-[15px] w-full py-3.5 rounded-xl font-medium bg-linear-to-b from-indigo-500 to-indigo-600 hover:from-indigo-700 disabled:cursor-not-allowed transition-colors">
