      ...
      <div className=" mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
        ...
        {loading && (
          <div className=" grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                className=" rounded-2xl bg-white/5 border border-white/10 animate-pulse h-[260px]"
                key={i}
              />
            ))}
          </div>
        ...
        {!loading && thumbnails.length === 0 && (
          <div className="py-24 text-center">
            ...
            <h3 className=" text-sm mt-2 text-zinc-400">
        ...
        {/* GRID */}
        {!loading && thumbnails.length > 0 && (
          <div className=" columns-1 sm:columns-2 2xl:columns-4 gap-8">
            {thumbnails.map((thumb: IThumbnail) => {
              ...
              return (
                <div
                  ...
                  className=" mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-2xl break-inside-avoid"
                >
                  {/* image */}
                  <div
                  ...
                  >
                    {thumb.image_url ? (
                      <img
                        src={thumb.image_url}
                        alt={thumb.title}
                        className=" w-full h-full object-cover group-hover:scalr-105 transition-transform duration-300"
                      />
                    ) : (
                  ...
                  <div onClick={(e)=> e.stopPropagation()} className=" absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                    <TrashIcon onClick={() => handleDelete(thumb._id)} className=" size-6 bg-black/50 p-1 rounded hover:bg-red-500 transition-all"/>
                    <DownloadIcon onClick={() => handleDownload(thumb.image_url!)} className=" size-6 bg-black/50 p-1 rounded hover:bg-green-500 transition-all" />
                      <Link to={`/preview?thumbnail_url=${thumb.image_url}&title=${thumb.title}`} target="_blank">
                    <ArrowUpRight className=" size-6 bg-black/50 p-1 rounded hover:bg-indigo-600 transition-all" />
                      </Link>
                  </div>
                </div>
              );
